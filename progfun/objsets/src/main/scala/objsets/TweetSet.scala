...
abstract class TweetSet {
  ...
  /**
    ...
    * and be implemented in the subclasses?
    */
  def descendingByRetweet: TweetList = if (this.empty) Nil
  else {
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    lazy val leftMR = left.mostRetweeted
    lazy val rightMR = right.mostRetweeted
    if (!left.empty) {
      if (!right.empty && rightMR.retweets > elem.retweets)
        if (rightMR.retweets > leftMR.retweets) rightMR else leftMR
      else if (elem.retweets > leftMR.retweets) elem else leftMR
    } else {
      if (!right.empty && rightMR.retweets > elem.retweets) rightMR
      else elem
    }
  }
